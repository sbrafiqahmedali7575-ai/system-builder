...
export function buildDailyConfirmationEmail(
  ...
  const tasks =
    details.tasks && details.tasks.length > 0
      ...
      : [
          {
            id: details.taskId || details.recordId,
            title: details.taskName || 'Daily Task',
  ...
  const subject = `System Builder • Review today’s ${tasks.length} task${tasks.length === 1 ? '' : 's'}`;

  const taskRows = tasks
    .map(
      (task) => `
        <tr>
          <td style="padding: 10px 12px; border-bottom: 1px solid #273449; vertical-align: top; width: 28px; font-size: 18px;">
            ${task.isCompleted ? '☑' : '☐'}
          </td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #273449; color: #f1f5f9; font-size: 14px; line-height: 1.45;">
...
<body style="margin:0;padding:0;background:#0f172a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#f8fafc;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background:#1e293b;border:1px solid #334155;border-radius:16px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:24px 26px;">
              <h1 style="margin:0 0 8px;font-size:21px;line-height:1.3;color:#fff;">Review today’s tasks</h1>
              <p style="margin:0 0 18px;color:#cbd5e1;font-size:14px;line-height:1.55;">
                ${completedCount} of ${tasks.length} currently completed. Open the secure review page, tick the tasks you completed, then submit once.
              </p>
              ...
              <p style="margin:16px 0 0;color:#94a3b8;font-size:12px;line-height:1.5;">
                For reliability and security, task checkboxes are submitted on the System Builder review page rather than directly inside the email.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 26px;border-top:1px solid #334155;background:#0f172a;text-align:center;color:#64748b;font-size:12px;">
              System Builder • Daily reminder • Asia/Kolkata
            </td>
          </tr>
        </table>
      </td>
  ...
  const text = `System Builder — Review today’s tasks
...
If every task is checked on submission, the day is marked Completed. Otherwise, the day is marked Not Completed.
`;
